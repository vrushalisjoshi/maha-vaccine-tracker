  ...
  if (arrMessages[district_id]) {
    while (arrMessages[district_id].length) {
      ...
      bot.telegram
        ...
        .then(() => {
          intSentMessagesCount++;
          console.log(`Sent: ${intSentMessagesCount}`);
        })
        .catch((err) => {
          console.log(`message not sent, added again: Pending Messages:${arrMessages[district_id].length}`);
